#!/bin/bash
# Build and prepare Mac OS 9 programs with C runtime support
# Usage: ./build_with_runtime.sh <test_name> [reference_binary]
#   test_name: Name without .c extension (e.g., beep_test_main)
...
if [ -z "$1" ]; then
    ...
    echo "Example: $0 beep_test_main"
    echo "Example: $0 beep_test_main beep_test_codewarrior"
    exit 1
fi
...
# - Links macos_classic_cxx.o (provides atexit, exit, C++ runtime)
# - Links macos_classic_qd.o (provides QuickDraw globals)
echo "1. Compiling and linking ${TEST_NAME}.c..."
cd "$TEST_DIR"
...
echo ""
echo "Features:"
echo "  • Uses standard main() function"
echo "  • Mac OS headers included automatically"
echo "  • Pascal strings enabled"
echo "  • return 0; calls ExitToShell()"
echo ""
