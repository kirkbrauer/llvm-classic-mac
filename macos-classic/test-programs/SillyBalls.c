...
#	QuickDraw.  It is about two pages of code, and does nothing more than
open #	a color window and draw randomly colored ovals in the window.
#
#	The purpose is to show how to get some initial results with Color
QuickDraw. #	It is a complete program and is very short to be as clear as
possible.
#
#	It does not have an Event Loop.  It is not fully functional in the sense
that #	it does not do all the things you would expect a well behaved Macintosh
#	program to do, like size the window naturally, have an event loop, use
menus, #	etc.
#
#	See Sample and TESample for the general structure and MultiFinder
techniques that #	we recommend that you use when building a new
application.
#
------------------------------------------------------------------------------*/
...
//						It draws colored balls in a
// color window, then uses colored
// text inverted in the ball.  The ball location and color is Random.
//
//						This program was written by Bo3b
// Johnson, 1/88.
//
//						The inverted LLVM text was a
...
#define LLVMSize 8 /* Size of text in each ball */
...
void Initialize(void) {
  ...
  //
  //	To make the Random sequences truly random, we need to make the seed
  // start 	at a different number.  An easy way to do this is to put the
  // current time 	and date into the seed.  Since it is always incrementing
  // the starting seed 	will always be different.  Don’t for each call of
  // Random, or the sequence 	will no longer be random.  Only needed once,
  // here in the init.
  //
  ...
  mainPtr = NewCWindow(nil, &windRect, "\pLLVM Land", true, documentProc,
                       (WindowPtr)-1, false, 0);

  SetPort(mainPtr);   /* set window to current graf port */
  TextSize(LLVMSize); /* smaller font for drawing. */
}
...
void NewBall(void) {
  ...
  //
  //	Make a Random new location for the ball, that is normalized to the
  // window size. 	This makes the Integer from Random into a number that is
  // 0..windRect.bottom 	and 0..windRect.right.  They are normalized so
  // that we don't spend most of our 	time drawing in places outside of the
  // window.
  //
  newTop = Random();
  newLeft = Random();
  ...
  //
  MoveTo(ballRect.left + BallWidth / 2 - LLVMSize,
         ballRect.top + BallHeight / 2 + LLVMSize / 2 - 1);

  //
  //	Invert the color and draw the text there.  This won’t look quite right
  // in 1 bit 	mode, since the foreground and background colors will be the
  // same. 	Color QuickDraw special cases this to not invert the color, to
  // avoid
  //	invisible drawing.
  //
